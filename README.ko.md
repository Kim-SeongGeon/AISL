...
- 지상 단일 로봇의 현재·과거 local object map을 비교하는 연구를 준비하기 위해, 입력과 정답을 직접 만들 수 있는 작은 실험을 구성하였다.
- 이번 실험은 센서나 ROS를 사용하는 실제 SLAM 실행이 아니라, 지도 생성·대응 후보 채점·좌표 변환 계산을 확인하는 Python 기초 실험이다.
- 학습용으로 2D 위치를 사용했으며, 최종 연구의 지도 표현을 2D로 확정한 것은 아니다.

**2. 지도 생성과 정답 설정**

- 과거 지도 A: 나무 4개, 기둥 3개, 자동차 3개로 총 10개 객체를 생성하였다. 위치는 x·y 각각 −5~5 m 범위에서 생성하였다.
- 현재 지도 B: A에 30° 회전과 (10, 5) m 평행이동을 적용하였다. 변환 방향은 A → B이다.
- B의 객체 순서를 섞어 두 지도의 배열 인덱스가 정답 대응을 그대로 알려주지 않도록 하였다.
- 매칭 함수에는 객체 위치와 class만 전달한다. 실제 객체의 동일성을 나타내는 정답 정보는 후보 채점 및 oracle 검증에 별도로 사용한다.
...
1. 가상 지도 A와 B 및 정답 대응을 생성한다.
2. 같은 semantic class의 객체를 모두 연결하는 class all-to-all 방식으로 후보를 만든다.
3. 후보 수, 후보 precision, 정답 대응 보존율(recall)을 계산한다.
4. 별도로 정답 대응을 이용해 2D least-squares 회전·이동을 복원하고 정답 변환과 비교한다.
5. 조건별 첫 시행의 지도 그림·JSON과 전체 시행의 CSV를 저장한다.

> 현재 후보 생성은 단순 기준 방법이며 SlideGraph 구현이 아니다. CLIPPER도 아직 연결하지 않았다. 아래 oracle 변환 오차는 정답 대응을 알고 수행한 계산 점검 결과이므로, 자동 매칭 정확도나 loop closure 성공률로 해석하면 안 된다.
...
- 각 조건은 seed 42~71로 30회 실행하여 총 210개 결과를 저장하였다.
- 조건 간 동일 seed에서는 같은 기본 지도를 사용한다. 추가 객체와 위치 잡음은 우선 별도 조건으로 적용하였다.
- 위치 잡음은 객체마다 x·y 좌표에 독립적으로 더하며 class는 유지한다.
- 원래 객체의 누락, 잘못된 class, 겹치지 않는 지도 쌍은 아직 포함하지 않았다.
...
- 처리 시간, 메모리, 실제 정합 성공률 및 CLIPPER 이후 지표는 이번 버전에서 측정하지 않았다.
...
- 기본 후보 수는 4×4 + 3×3 + 3×3 = 34개이며, 실제 정답 대응은 10개이다.
- 추가 객체가 늘면 오대응 후보가 증가하여 precision이 낮아졌다. Oracle 계산은 정답 대응만 사용하므로 추가 객체가 이 계산에 직접 들어가지는 않는다.
- 위치 잡음만으로 후보 수가 바뀌지 않는 이유는 현재 후보 생성이 위치를 사용하지 않고 class만 비교하기 때문이다.
- 모든 시행에서 recall 100%를 확인했다. 이는 원래 객체 10개와 class를 보존하고 같은 class를 전부 연결하는 설정의 결과이며, 새 descriptor의 강건성을 입증하는 결과가 아니다.
- 무잡음 조건의 모든 시행에서 oracle 위치·회전 오차가 각각 1e-9 m 및 1e-9° 미만인지 자동 검사하여 통과하였다.
- seed=42에서는 잡음 크기에 따라 oracle 오차도 증가했다. 전체 성능 경향을 판단하려면 반복 결과의 통계와 실제 매칭 결과를 추가로 분석해야 한다.
...
Python 표준 라이브러리만 사용하며 추가 패키지가 필요 없다. Python 3이 설치된 환경에서는 실험 폴더에서 다음과 같이 실행한다.
...
현재 PC에서는 프로젝트 루트에서 다음 실행 스크립트를 사용한다.
...
- 알려진 변환으로 가상 object map 쌍을 생성하고, 정답 대응 채점과 2D 변환 복원이 정상적으로 동작하는 기초 실험 환경을 마련하였다.
- 대응 후보를 많이 남기면 정답 보존율은 높일 수 있지만 오대응 후보도 늘어난다. 따라서 새 방법은 후보 수 감소와 정답 대응 보존을 함께 평가해야 한다.
- 실제 물체의 대응이 맞더라도 위치 잡음 때문에 추정 변환에 오차가 생길 수 있음을 oracle 예제로 확인하였다.
- 현재 결과는 SlideSLAM 재현이나 SlideGraph·CLIPPER 성능 검증, 신규 descriptor의 개선 입증이 아니다.
- 다음 단계는 기존 SlideGraph와 실제 CLIPPER를 연결하고, 동일 입력·후속 처리 조건에서 대응 품질, 정합 성공률, 변환 오차 및 단계별 처리 시간을 비교하는 것이다.
