...
| 1.0 m | 122.3개 | 51.40% | 낮음 | 벽과 기둥의 윤곽이 끊기고 작은 구조가 많이 소실됨 | 동작 자체는 정상이나 과도한 다운샘플링으로 구조 정보 손실이 큼 |
  ...
  - 결론: **filter_size_surf**가 증가하면서 평균 출력 포인트 수는 947.1개에서 122.3개로 크게 감소했다. CPU 사용률은 0.2m에서 가장 높았지만 0.5m와 1.0m의 차이는 거의 없었다. 따라서 이 데이터와 컴퓨터 환경에서 voxel 크기를 0.5m 이상으로 키워도 추가적인 CPU 절감 효과는 제한적인 반면, 지도 구조의 손실은 증가한 것으로 나타났다.
